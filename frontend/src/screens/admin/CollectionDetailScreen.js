...
const CollectionDetailScreen = ({ navigation, route }) => {
  ...
  const item = route?.params?.collection || {};
    ...
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.card}>
          ...
          <Text style={styles.name}>{item.farmerId?.fullName || item.farmerCode || '-'}</Text>
        ...
        <View style={styles.card}>
          ...
          <Row label="Shift" value={item.shift} />
          <Row label="Animal" value={item.animalType} />
          <Row label="Quantity" value={`${Number(item.quantityLiters || 0).toFixed(2)} L`} />
          <Row label="FAT" value={item.fat} />
          <Row label="SNF" value={item.snf} />
          <Row label="Rate/L" value={`₹${Number(item.ratePerLiter || 0).toFixed(2)}`} />
          <Row label="Amount" value={`₹${Number(item.amountInr || 0).toFixed(2)}`} />
