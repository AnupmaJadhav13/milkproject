        ...
        onSubmit={async (values) => {
          try {
            ...
            if (payload.collectionHead) {
              if (payload.collectionHead.password === '') delete payload.collectionHead.password;
              const hasHeadData = payload.collectionHead.username || payload.collectionHead.fullName || payload.collectionHead.mobileNumber;
              if (!hasHeadData) delete payload.collectionHead;
            }
            await dispatch(updateCenter({ id: center._id, data: payload, token })).unwrap();
            Toast.show({ type: 'success', text1: 'Success', text2: 'Collection center updated successfully', position: 'top', visibilityTime: 3000 });
            navigation.goBack();
          } catch (error) {
